...
router.post('/detect-face', function(req, res){
    ...
    if(!req.body.photo || typeof req.body.photo !== 'string'){
        res.send({ "found":false, "message":"Please pass valid image base64 string.(photo)" })
    }
...
/* Method - Index new face of user by photo.
   Recive - Photo in format base 64, Id_user is a external id of your control aplication.
...
router.post('/delete-face', function(req, res){
    if(!req.body.face_id || typeof req.body.face_id !== 'string'){
        res.send({ "found":false, "message":"Please pass valid face id.(face_id)" })
    }

    //recive param face_id, thats is a unique key of face.
    const obj = { "face_id": req.body.face_id }
